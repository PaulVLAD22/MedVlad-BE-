...
//                .build();
        ...
        for (Question question : questionLists) {
            ...
            symptoms.forEach(symptom -> binaryArray.set(symptomList.indexOf(symptom), 1));
            // binary array is the vector of a condition
            if (similarity(binaryArray, selectedSymptoms) == maxSimilarity) {
...
//
//
//        // diagnosis Result Dtos holds ever
//        for (Question question : questionLists) {
//            List<Symptom> symptoms = question.getSymptoms();
//            List<Integer> binaryArray = new ArrayList<>();
//            for (int i = 0; i < symptomList.size(); i++) {
...
//
//            // 1 SINGURA CONDITIE
//            if (similarity(binaryArray,selectedSymptoms,1)>0.8){
//                return diagnosisResultDtos.add(DiagnosisResultDto.builder()
//                        .condition(question.getAnswer().getPacientCondition())
//                        .similarityScore(maxSimilarity)
//                        .similarQuestionDoctor(question.getAnswer().getDoctor().userDtoFromUser())
//                        .build());
//            }
//        }
//        // adaugi conditia ce explica cele mai multe simptome si tai din ele
//        for (int i =0;i<questionLists.size()-1;i++){
//            for (int j =i+1;j<questionLists.size();j++){
//                List<Integer> binaryCombinationArray = //AFLA CE AR TRB SA FIE AICI
//            }
//        }
//        return diagnosisResultDtos;
//
...
//    //https://stats.stackexchange.com/questions/550857/which-method-is-best-to-find-the-correlation-between-2-datasets-in-my-case?noredirect=1&lq=1
//
//    private double similarity(List<Integer> vectorA, List<Integer> vectorB,int nrOfConditions) {
//        if (nrOfConditions==1)
//            return scalarProduct(vectorA,vectorB)/Collections.frequency(vectorA,1);
//        int maxVectorSum = Math.max(Collections.frequency(vectorA,1),Collections.frequency(vectorB,1));
//        return scalarProduct(vectorA,vectorB)/maxVectorSum;
//    }
//
//    private double scalarProduct(List<Integer> vectorA, List<Integer> vectorB){
//        int out=0;
